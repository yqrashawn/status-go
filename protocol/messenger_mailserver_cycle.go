...
func (m *Messenger) waitUntilMailserverAvailable() error {
	...
	if pinnedMailserver != nil {
		...
		if !ok {
			m.logger.Info("Pinned mailserver without info found, returning")
			return nil
		}
...
func (m *Messenger) allMailserversV1() ([]mailservers.Mailserver, error) {
	...
	m.logger.Info("pulling custom mailservers")
	for _, c := range customMailservers {
		m.logger.Info("Comparing custmo", zap.String("f1", c.Fleet), zap.String("f2", fleet))
		if c.Fleet == fleet {
...
func (m *Messenger) mailserverAddressToID(address string) (string, error) {
	allMailservers, err := m.allMailserversV1()
	if err != nil {
		return "", err
	}
		...
		case <-m.mailserverCycle.events:
			...
			for pID, pInfo := range m.mailserverCycle.peers {
				...
				for _, connectedPeer := range connectedPeers {
					m.logger.Info("IDS", zap.String("he", connectedPeer.Enode), zap.String("be", pInfo.mailserver.Address))
					if connectedPeer.Enode == pInfo.mailserver.Address {
						found = true
						break
					}
				}
			...
			for _, connectedPeer := range connectedPeers {
				id, err := m.mailserverAddressToID(connectedPeer.Enode)
				if err != nil {
					m.logger.Error("failed to convert id to hex", zap.Error(err))
					return
				}
				if id == "" {
					m.logger.Warn("id not found", zap.String("address", connectedPeer.Enode))
					continue
				}
				pInfo, ok := m.mailserverCycle.peers[id]
				if !ok || pInfo.status != connected {
					m.logger.Info("Peer connected", zap.String("peer", connectedPeer.Enode))
					pInfo.status = connected
					pInfo.canConnectAfter = time.Now().Add(defaultBackoff)

					if m.mailserverCycle.activeMailserver != nil && id == m.mailserverCycle.activeMailserver.ID {
						m.logger.Info("Mailserver available", zap.String("address", connectedPeer.Enode))
						signal.SendMailserverAvailable(m.mailserverCycle.activeMailserver.Address, m.mailserverCycle.activeMailserver.ID)
					}
